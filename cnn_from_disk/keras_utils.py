...
def keras_train_model(train_df,test_df,save_dir,model_name,save_test_over_th=True,model='InceptionResNetV2',model_kargs={'input_shape':(224,224,3),'include_top':False,'weights':'imagenet'},
  ...
  keras_compile_kargs={'loss':'categorical_crossentropy','loss_weights':None,'metrics':['accuracy',tp,tn,fp,fn]},
  keras_fit_kargs={'class_weight':None}):
    """
    ...
    #
    model.compile(optimizer=Adadelta(lr=learning_rate, rho=0.95, epsilon=None, decay=decay),**keras_compile_kargs)
